<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(!Auth::user()->isAnimator)
                        <x-nav-link :href="route('inschrijvingsdetails.index')"
                                    :active="request()->routeIs('inschrijvingsdetails.index')">
                            {{ __('Inschrijvingen') }}
                        </x-nav-link>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            ...
                            {{ Auth::user()->profiel->voornaam ?? 'Uitloggen' }}
                            <div class="ms-1">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('activiteiten.index')"
                                   :active="request()->routeIs('activiteiten.index')">
                {{ __('Activiteiten') }}
            </x-responsive-nav-link>
            @if(!Auth::user()->isAnimator)
                <x-responsive-nav-link :href="route('inschrijvingsdetails.index')"
                                       :active="request()->routeIs('inschrijvingsdetails.index')">
                    {{ __('Inschrijvingen') }}
                </x-responsive-nav-link>
            ...
            @endif
            @if(Auth::user()->isAdmin)
                <x-responsive-nav-link :href="route('profiel.index')" :active="request()->routeIs('profiel.index')">
                    {{ __('Ouders') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('profile.index')" :active="request()->routeIs('profile.index')">
                    {{ __('Gebruikers') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('locatie.index')" :active="request()->routeIs('locatie.index')">
                    {{ __('Locaties') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('uitpas.index')" :active="request()->routeIs('uitpas.index')">
                    {{ __('UiTPAS') }}
                </x-responsive-nav-link>
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div
                    class="font-medium text-base text-gray-800">{{ Auth::user()->profiel->voornaam ?? 'Afmelden' }}</div>
            </div>
